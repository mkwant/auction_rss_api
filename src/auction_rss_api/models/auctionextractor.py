...
class AuctionExtractorAsync(AuctionExtractor, ABC):
    """
    Async version of AuctionExtractor for Playwright-based scrapers.
    """
    ...
    async def get_auctions(self) -> List[Auction]:
        """
        Async version of auction scraping.
        Must be implemented by concrete async extractors.
        """
        ...

    async def search(self) -> RSSResponse:
        """
        Async version of search pipeline.
        """
        try:
            ...
            auctions = self.auctions_on_error(
                error_text="ERROR: Feed items couldn't be retrieved.",
                error=(
                    f"Received an error of type {type(e).__name__} "
                    f"when trying to retrieve the feed items:\n\n"
                    f"<pre>{traceback.format_exc()}</pre>"
                ),
            )

        logger.info(
            f"[{self.site_desc} ('{self.search_term}')] "
            f"Retrieved {len(auctions)} entries."
        )
        ...
        # Transformers (async)
        for transformer in self.transformers + self.default_transformers:
            await asyncio.gather(*(transformer(a) for a in auctions))
